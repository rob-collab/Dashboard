...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { policyId, notes } = await request.json().catch(() => ({}));
  if (!policyId) return errorResponse("policyId required", 400);
  try {
    await prisma.processPolicyLink.create({
      data: { processId: id, policyId, linkedBy: auth.userId, notes: notes ?? null },
    });
    ...
    console.error(e);
    return errorResponse("Failed to link policy", 500);
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const auth = await requireCCRORole(request);
  if ("error" in auth) return auth.error;
  const { policyId } = await request.json().catch(() => ({}));
  if (!policyId) return errorResponse("policyId required", 400);
  try {
    await prisma.processPolicyLink.deleteMany({ where: { processId: id, policyId } });
    const fresh = await prisma.process.findUnique({ where: { id }, include: PROCESS_INCLUDE });
    if (!fresh) return errorResponse("Not found", 404);
    const maturityScore = computeMaturity(fresh as Parameters<typeof computeMaturity>[0]);
    const result = await prisma.process.update({ where: { id }, data: { maturityScore }, include: PROCESS_INCLUDE });
    return jsonResponse(serialiseDates(result));
  } catch (e) { console.error(e); return errorResponse("Failed to unlink policy", 500); }
}
