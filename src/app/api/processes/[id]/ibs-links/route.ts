...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { ibsId, notes } = await request.json().catch(() => ({}));
  if (!ibsId) return errorResponse("ibsId required", 400);
  try {
    await prisma.processIBSLink.create({
      data: { processId: id, ibsId, linkedBy: auth.userId, notes: notes ?? null },
    });
    ...
    console.error(e);
    return errorResponse("Failed to link IBS", 500);
  }
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const auth = await requireCCRORole(request);
  if ("error" in auth) return auth.error;
  const { ibsId } = await request.json().catch(() => ({}));
  if (!ibsId) return errorResponse("ibsId required", 400);
  try {
    await prisma.processIBSLink.deleteMany({ where: { processId: id, ibsId } });
    const fresh = await prisma.process.findUnique({ where: { id }, include: PROCESS_INCLUDE });
    if (!fresh) return errorResponse("Not found", 404);
    const maturityScore = computeMaturity(fresh as Parameters<typeof computeMaturity>[0]);
    const result = await prisma.process.update({ where: { id }, data: { maturityScore }, include: PROCESS_INCLUDE });
    return jsonResponse(serialiseDates(result));
  } catch (e) { console.error(e); return errorResponse("Failed to unlink IBS", 500); }
}
