...
<body>
  ...
  <footer class="report-footer">
    ${branding?.logoSrc && branding.showInFooter ? `<div class="footer-logo"><img src="${branding.logoSrc}" alt="${escapeHtml(branding.logoAlt)}" style="width:${Math.round(branding.logoWidth * 0.75)}px;object-fit:contain" /></div>` : ""}
    <p>Generated by ${branding?.companyName ? escapeHtml(branding.companyName) : "Updraft"} CCRO Dashboard &bull; ${publishDate.split("T")[0]}</p>
...
function renderSection(section: Section): string {
  ...
  switch (section.type) {
    ...
    case "IMAGE_BLOCK": {
      const imgSrc = (content.src as string) || "";
      const imgAlt = escapeHtml((content.alt as string) || "");
      const imgCaption = (content.caption as string) || "";
      const imgWidth = content.width as number | null;
      const imgAlignment = (content.alignment as string) || "center";
      const imgFit = (content.objectFit as string) || "contain";
      ...
      const maxW = imgWidth ? `max-width:${imgWidth}px;` : "";
      if (!imgSrc) return `<section class="report-section" style="${style}">
        ${section.title ? `<h2>${escapeHtml(section.title)}</h2>` : ""}
        <div class="image-placeholder">No image</div>
      </section>`;
      return `<section class="report-section" style="${style}">
        ${section.title ? `<h2>${escapeHtml(section.title)}</h2>` : ""}
        <figure style="${alignStyle};${maxW}display:flex;flex-direction:column">
          <img src="${imgSrc}" alt="${imgAlt}" style="width:100%;object-fit:${imgFit};border-radius:0.5rem" />
          ${imgCaption ? `<figcaption style="margin-top:0.5rem;text-align:center;font-size:0.75rem;color:#6b7280;font-style:italic">${escapeHtml(imgCaption)}</figcaption>` : ""}
        </figure>
      </section>`;
    }
